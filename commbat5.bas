...
SP1% = INSTR(2, XXYY$, " ")
YY1% = VAL(LEFT$(XXYY$, SP1%))
XX1% = VAL(MID$(XXYY$, SP1% + 1))
...
CASE UP:    EL$ = BigMap$(RealY% - 1, RealX%): IF EL$ = "*" OR VAL(EL$) > 0 THEN Badmove% = 3: GOTO BadOut
CASE Down:  EL$ = BigMap$(RealY% + 1, RealX%): IF EL$ = "*" OR VAL(EL$) > 0 THEN Badmove% = 3: GOTO BadOut
CASE Left:  EL$ = BigMap$(RealY%, RealX% - 1): IF EL$ = "*" OR VAL(EL$) > 0 THEN Badmove% = 3: GOTO BadOut
CASE Right: EL$ = BigMap$(RealY%, RealX% + 1): IF EL$ = "*" OR VAL(EL$) > 0 THEN Badmove% = 3: GOTO BadOut
END SELECT
...
REM U% = VAL(J$): IF U% THEN ETank% = U%: GOTO doinout
Paramter% = 0
...
IF Cmdin$ <> "SM" THEN Parameter% = VAL(Parameter$)
		...
		SELECT CASE Cmdin$
 CASE "TU": CALL EraseET: ETankY%(ETank%) = ETankY%(ETank%) - 1: ETankM%(ETank%) = Parameter%: CALL GetSod
 CASE "TD": CALL EraseET: ETankY%(ETank%) = ETankY%(ETank%) + 1: ETankM%(ETank%) = Parameter%: CALL GetSod
 CASE "TL": CALL EraseET: ETankX%(ETank%) = ETankX%(ETank%) - 1: ETankM%(ETank%) = Parameter%: CALL GetSod
 CASE "TR": CALL EraseET: ETankX%(ETank%) = ETankX%(ETank%) + 1: ETankM%(ETank%) = Parameter%: CALL GetSod
 ...
 CASE ELSE: PRINT J$, TCInStat
		END SELECT
REM PRINT ETankX%(1), ETankY%(1), ETankM%(1);
doinout:
CALL Views
END SUB
...
SUB EraseET
CALL convert(ETankY%(ETank%), ETankX%(ETank%), ETankM%(ETank%))
BigMap$(RealY%, RealX%) = ETwas$(ETank%)
