...
DIM SHARED ETankM%(8), ETwas$(8), LazerRange%, ComPort%

DIM Loopy%, Loopx%, Keyin%
INPUT "Which Com Port To Use"; Com$: IF Com$ = "" THEN Com$ = "2"
ComPort% = VAL(Com$)
CONST UP = 1
CONST Down = 2
CONST Left = 3
...
FOR ET% = 1 TO 8
ETankX%(ET%) = Rmd(40): ETankY%(ET%) = Rmd(20): ETankM%(ET%) = Rmd(8)
CALL convert(ETankY%(ET%), ETankX%(ET%), ETankM%(ET%))
ETwas$(ET%) = BigMap$(RealY%, RealX%)
BigMap$(RealY%, RealX%) = "*"
NEXT
CALL Msg("")
END SUB
